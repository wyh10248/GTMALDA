...
def adjacency_matrix_to_edge_index(adjacency_matrix):
   # 将 NumPy 数组转换为 PyTorch 张量
     adjacency_matrix_tensor = torch.from_numpy(adjacency_matrix)
    
     # 使用 torch.nonzero() 获取非零元素的索引
     edge_index = torch.nonzero(adjacency_matrix_tensor, as_tuple=False).t()

     return edge_index.to(torch.float32)
...
def gen_folds(adj):
    ...
    for train, test in kf.split(positive_idx):
        ...
        positive5foldsidx.append(positive_train_idx)

    for train, test in kf.split(negative_idx[0 : len(positive_idx)]):
        negative_train_idx = negative_idx[train]
        negative5foldsidx.append(negative_train_idx)

    for i in range(len(positive5foldsidx)):
        # for i in range(1):
        train_mask = np.zeros_like(adj, dtype=int)
        train_fold_idx = np.concatenate(
            (pos_ij[positive5foldsidx[i]], neg_ij[negative5foldsidx[i]])
        )
